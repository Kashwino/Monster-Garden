...
commands=[['--headless','--editor','--path',str(root),'--import']]+[['--headless','--path',str(root),'--script',s] for s in (sys.argv[1:] or ['tests/run_tests.gd','tests/phase2_tests.gd'])]
failed=False
for i,args in enumerate(commands):
 p=subprocess.run([godot]+args,env=env,text=True,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,timeout=90)
