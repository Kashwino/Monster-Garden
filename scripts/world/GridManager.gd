...
func _build() -> void:
	...
	for i: int in Game.plots.size():
		...
		_plants.append(null)
		_stages.append("")
		_owned.append(not bool(Game.plots[i].unlocked))
...
func _refresh_plot(index: int) -> void:
	...
	if _owned[index]!=bool(Game.plots[index].unlocked):
		var old:=holder.get_node_or_null("Soil")
		if old!=null: holder.remove_child(old);old.queue_free()
		var soil:=AssetFactory.create_plot(bool(Game.plots[index].unlocked))
		soil.name="Soil";holder.add_child(soil)
		_owned[index]=bool(Game.plots[index].unlocked)
	_update_plant(index)
...
func _update_plant(index: int) -> void:
	...
	var key:=String(plot.species_id)+str(plot.planted_at)+JSON.stringify(plot.genes)
	if stage != _stages[index] or (is_instance_valid(_plants[index]) and _plants[index].get_meta("key","")!=key):
	...
	if not bool(plot.unlocked):
		label.text = "Lv.%d" % Game.plot_unlock_level(index) if index == Game.selected_plot else ""
	elif plot.species_id == "":
		label.text = "+"
	elif stage == "blooming":
		label.text = "READY" if index==Game.selected_plot else "◆"
	else:
		label.text = ("%ds" % Game.remaining(index) if Game.remaining(index) < 60 else "%dm" % int(ceil(Game.remaining(index) / 60.0))) if index==Game.selected_plot else ""
